"""
Render "KV" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
def _default_font():
    """Pick a bold system font that exists on the current OS."""
    candidates = [
        ("/System/Library/Fonts/Futura.ttc", 2),          # macOS
        ("C:/Windows/Fonts/arialbd.ttf", 0),              # Windows – Arial Bold
        ("C:/Windows/Fonts/impact.ttf", 0),               # Windows – Impact
        ("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 0),  # Linux
    ]
    for path, idx in candidates:
        if os.path.isfile(path):
            return path, idx
    return candidates[0]  # fall through; will error with a clear message

_fp, _fi = _default_font()
FONT_PATH = os.environ.get("WORDMARK_FONT", _fp)
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", _fi))   # face within a .ttc
...
TEXT = os.environ.get("WORDMARK_TEXT", "KV")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">Kartikey@github: ~$ ./wordmark.sh --3d</text>')
